...
![Screenshot](https://github.com/thelabcat/word-search-generator/blob/master/qt_main_window_screenshot.png?raw=true "The main app window, Qt version")
...
There are two main ways to run this program. You can download a bundled executable, or install the PyPi package. The bundled executables will have slower launch times, but are easier to set up.

## Executable download

[Download EXE](https://github.com/thelabcat/word-search-generator/releases/latest/download/wordsearchgen-win-x86_64.exe)    [Download for Linux](https://github.com/thelabcat/word-search-generator/releases/latest/download/wordsearchgen-linux-x86_64)

These are portable executables. Double-click them to run. They will not install anything.
...
To install and run from the PYPI package:
1. Download and install a version of Python 3 if it is not already installed. On Linux, it probably is.
2. If you are _not_ on Microsoft Windows, you will likely need to install Python 3 Pip separately. Do so now.
3. Install the package with `pip install word-search-app`. If this fails with an error about "This environment is externally managed", you must create a **Python virtual evnironment** and activate it, then try again.
4. Launch the app by running `wordsearchgen` as a system command.

## Running manually from source

If you intend to run from manually downloaded source code, for example if you intended to fork the project, this program depends on the following Python libraries:
- [NumPy](https://pypi.org/project/numpy/)
- [PySide6](https://pypi.org/project/pyside6/) (weak dependency, needed only for the Qt version of the GUI)

Run the file `src/wordsearchgen_wrap.py` to start the app.

## Using the app
### The GUI

Once you launch the app, within a minute, a simple GUI with a text entry area should appear:
...
### Workflow for the CLI

The simplest way to use the program from the terminal is to just type all the words after it, with spaces in between each one:
```bash
wordsearchgen all your base are belong to us
```
...
One thing I didn't do was add native file output, but you can do it in BASh with an output redirection:
```bash
wordsearchgen one Mediator between God and men > puzzle.txt
```

You can also pipe words into the program, by using a single hyphen for the WORDS argument:
```bash
cat all_us_states_nospaces.txt | wordsearchgen -a - > all_states_puzzle.txt
```
